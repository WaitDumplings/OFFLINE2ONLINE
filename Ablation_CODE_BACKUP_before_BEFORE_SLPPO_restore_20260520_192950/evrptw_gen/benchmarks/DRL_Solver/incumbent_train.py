...
def _policy_loss_from_adv(adv, ratio, valid, clip_coef):
    return _policy_loss_from_adv_asym(adv, ratio, valid, clip_coef, clip_coef)[0]


def _policy_loss_from_adv_asym(adv, ratio, valid, clip_low, clip_high):
    valid_count = valid.sum().float().clamp_min(1.0)
    clip_low = max(float(clip_low), 0.0)
    clip_high = max(float(clip_high), 0.0)
    clipped_ratio = torch.clamp(ratio, 1.0 - clip_low, 1.0 + clip_high)
    pg1 = -adv * ratio
    pg2 = -adv * clipped_ratio
    loss = (torch.max(pg1, pg2) * valid).sum() / valid_count
    ...
    with torch.no_grad():
        ...
        if valid_ratio.numel() == 0:
            stats = {
                ...
                "pos_adv_frac": 0.0,
                "neg_adv_frac": 0.0,
                "pos_clip_high_frac": 0.0,
                "neg_clip_low_frac": 0.0,
            }
        else:
            low_mask = valid_ratio < (1.0 - clip_low)
            high_mask = valid_ratio > (1.0 + clip_high)
            pos_mask = valid_adv > 0
            neg_mask = valid_adv < 0
            stats = {
                "ratio_mean": float(valid_ratio.mean().detach().cpu().item()),
                "ratio_std": float(valid_ratio.std(unbiased=False).detach().cpu().item()) if valid_ratio.numel() > 1 else 0.0,
                "clip_low_frac": float(low_mask.float().mean().detach().cpu().item()),
                "clip_high_frac": float(high_mask.float().mean().detach().cpu().item()),
                "pos_adv_frac": float(pos_mask.float().mean().detach().cpu().item()),
                "neg_adv_frac": float(neg_mask.float().mean().detach().cpu().item()),
                "pos_clip_high_frac": float((high_mask & pos_mask).float().sum().detach().cpu().item() / max(float(pos_mask.float().sum().detach().cpu().item()), 1.0)),
                "neg_clip_low_frac": float((low_mask & neg_mask).float().sum().detach().cpu().item() / max(float(neg_mask.float().sum().detach().cpu().item()), 1.0)),
            }
    return loss, stats
...
def _ppo_update(agent, optim_backbone, optim_critic, envs, rollout, advantages, returns, args):
    ...
    pos_clip_high_fracs = []
    neg_clip_low_fracs = []
    archive_clip_low_fracs = []
    archive_clip_high_fracs = []
    grad_cos_info = {}
    ...
    for _epoch in range(args.update_epochs):
        ...
        for start in range(0, num_envs, envsperbatch):
            ...
            clip_mode = str(getattr(args, "ppo_clip_mode", "symmetric")).lower()
            if clip_mode == "archive_asym":
                env_adv_full = b_adv_parts.get("base_actor", torch.zeros_like(b_advantages))
                env_adv = env_adv_full[mb_inds]
                archive_adv = torch.zeros_like(env_adv)
                for key in ("group_contrib", "ref_contrib", "cmp_contrib"):
                    part = b_adv_parts.get(key, None)
                    if part is not None:
                        archive_adv = archive_adv + part[mb_inds]
                env_loss, env_clip_stats = _policy_loss_from_adv_asym(
                    env_adv,
                    ratio,
                    mb_valid,
                    float(args.clip_coef),
                    float(args.clip_coef),
                )
                archive_loss, archive_clip_stats = _policy_loss_from_adv_asym(
                    archive_adv,
                    ratio,
                    mb_valid,
                    float(args.archive_clip_low),
                    float(args.archive_clip_high),
                )
                pg_loss = env_loss + archive_loss
                clip_stats = env_clip_stats
                archive_clip_low_fracs.append(float(archive_clip_stats["clip_low_frac"]))
                archive_clip_high_fracs.append(float(archive_clip_stats["clip_high_frac"]))
            else:
                if clip_mode == "asym":
                    clip_low = float(args.ppo_clip_low)
                    clip_high = float(args.ppo_clip_high)
                else:
                    clip_low = float(args.clip_coef)
                    clip_high = float(args.clip_coef)
                pg_loss, clip_stats = _policy_loss_from_adv_asym(
                    mb_adv,
                    ratio,
                    mb_valid,
                    clip_low,
                ...
                archive_clip_low_fracs.append(0.0)
                archive_clip_high_fracs.append(0.0)
            ratio_means.append(float(clip_stats["ratio_mean"]))
            ratio_stds.append(float(clip_stats["ratio_std"]))
            clip_low_fracs.append(float(clip_stats["clip_low_frac"]))
            clip_high_fracs.append(float(clip_stats["clip_high_frac"]))
            pos_adv_fracs.append(float(clip_stats["pos_adv_frac"]))
            neg_adv_fracs.append(float(clip_stats["neg_adv_frac"]))
            pos_clip_high_fracs.append(float(clip_stats["pos_clip_high_frac"]))
            neg_clip_low_fracs.append(float(clip_stats["neg_clip_low_frac"]))
    ...
    return {
        ...
        "clip_low_frac": float(np.mean(clip_low_fracs)) if clip_low_fracs else 0.0,
        "clip_high_frac": float(np.mean(clip_high_fracs)) if clip_high_fracs else 0.0,
        "pos_adv_frac": float(np.mean(pos_adv_fracs)) if pos_adv_fracs else 0.0,
        "neg_adv_frac": float(np.mean(neg_adv_fracs)) if neg_adv_fracs else 0.0,
        "pos_clip_high_frac": float(np.mean(pos_clip_high_fracs)) if pos_clip_high_fracs else 0.0,
        "neg_clip_low_frac": float(np.mean(neg_clip_low_fracs)) if neg_clip_low_fracs else 0.0,
        "archive_clip_low_frac": float(np.mean(archive_clip_low_fracs)) if archive_clip_low_fracs else 0.0,
        "archive_clip_high_frac": float(np.mean(archive_clip_high_fracs)) if archive_clip_high_fracs else 0.0,
...
def _print_update_summary(update_step, args, rollout, ppo_info, aux_info, archive_info, regret_info, group_meta, lambda_fail):
    ...
    print(
        ...
        f"kl={ppo_info['kl']:.5f} "
        f"ratio={ppo_info.get('ratio_mean', 1.0):.3f}/{ppo_info.get('ratio_std', 0.0):.3f} "
        f"clip={ppo_info.get('clip_low_frac', 0.0):.3f}/{ppo_info.get('clip_high_frac', 0.0):.3f} "
        f"archclip={ppo_info.get('archive_clip_low_frac', 0.0):.3f}/{ppo_info.get('archive_clip_high_frac', 0.0):.3f} "
        f"posneg={ppo_info.get('pos_adv_frac', 0.0):.3f}/{ppo_info.get('neg_adv_frac', 0.0):.3f} "
        f"sl=({sl_text}) "
        f"aux=({aux_text}) "
...
def parse_args():
    ...
    parser.add_argument("--ppo-clip-mode", type=str, default="symmetric", choices=["symmetric", "asym", "archive_asym"])
    parser.add_argument("--ppo-clip-low", type=float, default=0.20)
    parser.add_argument("--ppo-clip-high", type=float, default=0.20)
    parser.add_argument("--archive-clip-low", type=float, default=0.20)
    parser.add_argument("--archive-clip-high", type=float, default=0.40)
    ...
    for name in ("ppo_clip_low", "ppo_clip_high", "archive_clip_low", "archive_clip_high"):
        if float(getattr(args, name)) < 0.0:
            raise ValueError(f"{name} must be non-negative.")
