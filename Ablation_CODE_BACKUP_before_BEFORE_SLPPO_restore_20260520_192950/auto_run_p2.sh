...
if [[ "$CONDA_ENV" == "base" && "${ALLOW_BASE_PYTHON:-0}" != "1" ]]; then
  ...
  echo "Example: PYTHON_BIN=/home/npg0/anaconda3/envs/maojie/bin/python SEED=$SEED NUM_UPDATES=$NUM_UPDATES ./auto_run_p2.sh"
  exit 2
fi
...
nohup "$PYTHON_BIN" run_p2.py --seed "$SEED" --num-updates "$NUM_UPDATES" --python-bin "$PYTHON_BIN" 2>&1 | tee "$LAUNCH_LOG"
