...
class EVRPTWVectorEnv(gym.Env):
    ...
    def _STEP(self, action):
        ...
        if compute_repair_progress_pbrs:
            prev_repair_ratio = self._remaining_customer_repair_ratio()

        self._go_to(action)

        if compute_repair_progress_pbrs:
            post_repair_ratio = self._remaining_customer_repair_ratio()
        ...
        if self.env_mode == "train":
            ...
            if self.lag_reward:
                ...
                if self.use_repair_fail_reward:
                    ...
                    if np.any(fail_mask):
                        repair_dist = self._remaining_customer_repair_ratio()
