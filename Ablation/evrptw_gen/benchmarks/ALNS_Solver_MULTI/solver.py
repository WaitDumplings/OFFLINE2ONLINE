...
class ALNS_Solver:
    ...
    def __init__(
        ...
        self.format = format
        self.search_profile = str(search_profile or "").strip().lower()
        ...
        self.lambda_div = 0.0
        self.warm_start_bootstrap_iters = 0
        self._warm_cr_cycle: List[str] = []
        self._warm_ci_cycle: List[str] = []
        self._warm_sr_cycle: List[str] = []
        self._warm_si_cycle: List[str] = []

        self._apply_search_profile(self.search_profile)
    ...
    def _apply_search_profile(self, profile: str):
        profile = str(profile or "").strip().lower()
        if profile not in {"warm_1k", "online_warm_1k"}:
            return

        self.search_profile = profile
        self.max_iters = max(int(self.max_iters), 1000)

        # Warm-start runs only get a small iteration budget, so every operator
        # family should be tried before adaptive weights have enough evidence.
        self.NC = 50
        self.NS = 250
        self.NSR = 25
        self.NRR = 250
        self.nRR = 40
        self.warm_start_bootstrap_iters = 250

        self.min_remove_customers = max(1, min(int(0.12 * max(1, self.n_customers)), 30))
        self.max_remove_customers = max(
            self.min_remove_customers,
            min(int(0.35 * max(1, self.n_customers)), 60),
        )
        self.route_removal_upper_ratio = 0.35
        self.reaction_factor = 0.35

        self._warm_cr_cycle = list(self.cr_ops.keys())
        self._warm_ci_cycle = list(self.ci_ops.keys())
        self._warm_sr_cycle = list(self.sr_ops.keys())
        self._warm_si_cycle = list(self.si_ops.keys())
    ...
    def get_checkpoint(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "search_profile": str(self.search_profile),
            "warm_start_bootstrap_iters": int(self.warm_start_bootstrap_iters),
    ...
    def load_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        ckpt_profile = str(checkpoint.get("search_profile", "") or "").strip().lower()
        if ckpt_profile and ckpt_profile != self.search_profile:
            self._apply_search_profile(ckpt_profile)
    ...
    def solve(
        ...
        for it in (range(start_iter, target_iter + 1)):
            ...
            if it % self.NSR == 0:
                if (
                    self.warm_start_bootstrap_iters > 0
                    and it <= self.warm_start_bootstrap_iters
                    and self._warm_sr_cycle
                    and self._warm_si_cycle
                ):
                    station_idx = max(0, (it // max(1, self.NSR)) - 1)
                    sr_name = self._warm_sr_cycle[station_idx % len(self._warm_sr_cycle)]
                    si_name = self._warm_si_cycle[station_idx % len(self._warm_si_cycle)]
                else:
                ...
                if (
                    self.warm_start_bootstrap_iters > 0
                    and it <= self.warm_start_bootstrap_iters
                    and self._warm_cr_cycle
                    and self._warm_ci_cycle
                ):
                    cr_name = self._warm_cr_cycle[(it - 1) % len(self._warm_cr_cycle)]
                    ci_name = self._warm_ci_cycle[
                        ((it - 1) // len(self._warm_cr_cycle)) % len(self._warm_ci_cycle)
                    ]
                else:
                    if it % self.NRR == 0:
